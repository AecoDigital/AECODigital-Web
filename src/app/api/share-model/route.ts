  ...
  const { error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
        <div style="background:#00E5A3;padding:24px 32px;border-radius:12px 12px 0 0">
        ...
        <div style="background:#ffffff;padding:32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px">
          ...
          <a href="${link}" style="display:inline-block;background:#00E5A3;color:#ffffff;text-decoration:none;padding:12px 28px;border-radius:8px;font-weight:600;font-size:14px">
            Abrir modelo 3D
          </a>
          <p style="color:#9ca3af;font-size:12px;margin:24px 0 0;line-height:1.6">
            Si el botón no funciona, copia este enlace en tu navegador:<br/>
            <a href="${link}" style="color:#00E5A3;word-break:break-all">${link}</a>
          </p>
          <hr style="border:none;border-top:1px solid #f3f4f6;margin:24px 0"/>
          <p style="color:#9ca3af;font-size:11px;margin:0">
            Compartido desde <a href="https://aecodigital.com/bim-viewer" style="color:#00E5A3">aecodigital.com/bim-viewer</a>
          </p>
        </div>
      </div>
    `,
